...
def SPT3G_phi_Fisher(params,fsky,saveas,
    ...
    spec = m.get_cmb_fgs_and_cals(cov_params)[0]['PP'][:lmax+1]
    ell=arange(lmax+1)
...
def Planck_Fisher(params,lmax,saveas):
    ...
    for k,v in params.items():
        if isinstance(v,fish_param):
            ...
            cov_params[k]=v
    pcov=create_basic_cov(.7,[.0003],[0,'ukarc'],model='classy',specrange = (2,lmax),bin_size = 5,
...
model_extensions=[('lcdm',{}),
                   ...
                   ('lcdm_dmdr',{'N_drf':fish_param(.6,0.1),
                                  'Gamma0_dmdrf':fish_param(1.69,0.4),
                                 'gauge':'new'}),]
                #   ('lcdm_cdi',{'ic':'ad,cdi',
...
#    SPT3G_Fisher(params,0.7,'Saved_Fisher_Matrices/perfect'+k[0]+'_lmin50_Fisher_Matrices_theta.h5',
#    	beam_width=array([.00001]),
#                  noise=[array([.1]),'ukarc'],
#                  lmin=50,
#                  spectra=['TT','EE','TE'])
...
for k in model_extensions:
   print(k[0])
   params=dict(params_base,**k[1])
   SPT3G_Fisher(params,0.035,'Saved_Fisher_Matrices/perfect_patch'+k[0]+'_lmin50_Fisher_Matrices_theta.h5',
   	beam_width=array([3e-5]),
                 noise=[array([.1]),'ukarc'],
                 lmin=50,
                 spectra=['TT','EE','TE'])
...
#    print(k[0])
#    params=dict(params_base,**k[1])
#    SPT3G_Fisher(params,0.06,'Saved_Fisher_Matrices/perfect_patch'+k[0]+'_Fisher_Matrices_theta.h5',
#    	beam_width=array([.00001]),
#                  noise=[array([.1]),'ukarc'],
#                  lmin=2,
#                  spectra=['TT','EE','TE'])
...
#    print(k[0])
#    params=dict(params_base,**k[1])
#    SPT3G_Fisher(params,0.7,'Saved_Fisher_Matrices/perfect'+k[0]+'_Fisher_Matrices_theta.h5',
#    	beam_width=array([.00001]),
#                  noise=[array([.1]),'ukarc'],
#                  lmin=2,
#                  spectra=['TT','EE','TE'])
...
#    print(k[0])
#    params=dict(params_base,**k[1])
#    Planck_Fisher(params,800,'Saved_Fisher_Matrices/planck_TT_'+k[0]+'_800_Fisher_Matrix_theta.h5')

# for k in model_extensions:
#    print(k[0])
#    params=dict(params_base,**k[1])
#    Planck_Fisher(params,1400,'Saved_Fisher_Matrices/planck_TT_'+k[0]+'_approx_full_Fisher_Matrix_theta.h5')
...
for i,k in enumerate(model_extensions):
    ...
    f={}
    #Perfect
    f['perfect']=pd.read_hdf(
        'Saved_Fisher_Matrices/perfect'+k[0]+'_Fisher_Matrices_theta.h5', 'key')
    f['perfect_lmin50']=pd.read_hdf(
        'Saved_Fisher_Matrices/perfect'+k[0]+'_lmin50_Fisher_Matrices_theta.h5', 'key')
    f['perfect_patch_lmin50']=pd.read_hdf(
        'Saved_Fisher_Matrices/perfect_patch'+k[0]+'_lmin50_Fisher_Matrices_theta.h5', 'key')
    f['perfect_patch']=pd.read_hdf(
        'Saved_Fisher_Matrices/perfect_patch'+k[0]+'_Fisher_Matrices_theta.h5', 'key')
    #SPT3G
    ...
    f['Planck_TT_approx']=pd.read_hdf(
        'Saved_Fisher_Matrices/planck_TT_'+k[0]+'_approx_full_Fisher_Matrix_theta.h5', 'key')
